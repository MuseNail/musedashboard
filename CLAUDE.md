...
> - **§13 Worker auth is BUILT (v4.86, 2026-06-12) — enforcement pending owner rollout.** Shared bearer token: Worker `appAuthOk` gates EVERY route once the `APP_AUTH_TOKEN` secret is set (exempt: `/terminal/webhook` HMAC, `/gcal/callback`, `GET /photos/*`); clients send it via the `js/app/apptoken.js` fetch wrapper (`?auth=` on the WebSocket + `/gcal/connect`); token is device-local `muse_auth_token`, provisioned in Settings → Staff & Access → Device Access or `#auth=` setup links. Rollout steps in `PRIORITIES.md` #2.
...
| `muse_auth_token` | §13 backend access token for THIS device (never synced — provisioned via Settings → Device Access or an `#auth=` setup link) |
