...
export default {
  ...
  async _handle(request, env) {
    ...
    // POST /report/clear empties it. Forwarded to the DO like /state and /push.
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/report' && request.method === 'POST') {
      ...
      if (entry) {
        ...
        entry = {
          fingerprint: fp,
          app:     String(body.app || 'muse').slice(0, 20),
        ...
        if (isNew || serious) {
          ...
          if (now - last > 3600000) {
            ...
            await this.sendPushToTech('errors', {
              title: serious ? '⚠️ Muse — a failure was logged' : 'Muse — a problem was logged',
    ...
    try {
      switch (op) {
        ...
        case 'queue.entryPatch': {
          ...
          // fees/items/discount edit the way a whole-entry queue.upsert would.
          const e = await this.state.storage.get('queue:' + payload.entryId);
          if (!e) break;
