...
7. On next page load, each browser session detects the version change via `checkAppVersion()`, unregisters the service worker, and reloads once so all files are fetched fresh from the network. If the `↻` badge appears after reload, do a hard refresh (`Ctrl+Shift+R`) — that always works
